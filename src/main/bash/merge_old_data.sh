...
#ogr2ogr -lco ENCODING=UTF-8 -f "ESRI Shapefile" "/tmp/sektory_2020_pom.shp" PG:"$CON_STRING_OGR" "public.sektory_2020_pom" -overwrite

#cd /home/jencek/Documents/Projekty/PCR/github/Patrac/src/main/python/rename
#python3 rename.py
#mv sectors_pom_9_renamed.csv /tmp/sektory_2020_pom_labels.csv

#create table sektory_2020_pom_labels (id int, label varchar(10));
#copy sektory_2020_pom_labels from '/tmp/sektory_2020_pom_labels.csv' csv delimiter ';';
#alter table sektory_2020_pom add column label varchar(6);
#update sektory_2020_pom s set label = l.label from sektory_2020_pom_labels l where s.id = l.id::varchar;
#alter table sektory_2020_pom rename column type to typ;
#alter table sektory_2020_pom drop column id;
#alter table sektory_2020_pom drop column gid;
#alter table sektory_2020_pom add column id serial;
...
for kraj in $( cat kraje_list.txt ); do
  ...
  echo "FROM sektory_2020_pom s" >> 1.sql
