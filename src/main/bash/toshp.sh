...
aws s3 cp s3://patrac-data/docker-inputs/limits.txt limits.txt

LIMIT1=`cat limits.txt | cut -d' ' -f1`
LIMIT2=`cat limits.txt | cut -d' ' -f2`

while read p; do
  echo "$p"
  echo "DROP VIEW IF EXISTS cuzk_zbgd_cur_type;" > 1.sql
  echo "CREATE VIEW cuzk_zbgd_cur_type AS SELECT * FROM gdb."$p" WHERE ST_Within(geometry, (SELECT ST_Buffer(geom, $LIMIT1) FROM public.nu3 WHERE nu3 = '"$1"')) OR ST_Intersects(geometry, (SELECT ST_Buffer(geom, $LIMIT1) FROM public.nu3 WHERE nu3 = '"$1"'));" >> 1.sql
...
echo "CREATE VIEW cuzk_zbgd_buffer AS SELECT ST_Buffer(geom, $LIMIT1) FROM public.nu3 WHERE nu3 = '"$1"';" >> 1.sql
psql "$CON_STRING" -f 1.sql
ogr2ogr -lco ENCODING=UTF-8 -f "ESRI Shapefile" "/tmp/buffer.shp" PG:"$CON_STRING_OGR" "public.cuzk_zbgd_buffer" -overwrite

echo "DROP VIEW IF EXISTS cuzk_zbgd_buffer;" > 1.sql
echo "CREATE VIEW cuzk_zbgd_buffer AS SELECT ST_ExteriorRing(ST_Buffer(geom, $LIMIT2)) FROM public.nu3 WHERE nu3 = '"$1"';" >> 1.sql
psql "$CON_STRING" -f 1.sql
